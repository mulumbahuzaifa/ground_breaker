...
! function($) {
    ...
    vcGridStyleAll.prototype.init = function() {
        ...
        var $els = this.$content.find(".vc_grid-item" + this.filterValue),
            animation = (this.setIsLoading(), this.$content.closest(".vc_grid-container").data("initial-loading-animation"));
        ...
        html && html.length && (html = $(html), this.$el.html(html), this.unsetIsLoading(), this.$content = html.find('[data-vc-grid-content="true"]'), this.grid.initFilter(), this.filter(), window.vc_prettyPhoto())
...
function($) {
    ...
    vcGridStyleLoadMore.prototype.setIsLoading = function() {
        ...
        this.$el = this.grid.$el, this.$content = this.$el, this.grid.initFilter(), this.filter(void 0, !0), this.showItems(!0), this.filterValue = -1, window.vc_prettyPhoto(), this.$content.append(this.$loader), this.grid.ajax({}, this.addItems)
    }, vcGridStyleLoadMore.prototype.showItems = function(animate) {
        var $els = this.$content.find(".vc_grid_filter-item:not(.vc_visible-item):lt(" + this.settings.items_per_page + ")"),
            animation = (this.setIsLoading(), this.$content.closest(".vc_grid-container").data("initial-loading-animation"));
        window.vcGridSettings.addItemsAnimation = animation, $els.addClass("vc_visible-item"), animate && $els.addClass(window.vcGridSettings.addItemsAnimation + " animated"), this.unsetIsLoading(), $(window).trigger("grid:items:added", this.$el)
    }, vcGridStyleLoadMore.prototype.filter = function(filter, animate) {
        if (filter = _.isUndefined(filter) || "*" === filter ? "" : filter, this.filterValue == filter) return !1;
        this.$content.find(".vc_visible-item, .vc_grid_filter-item").removeClass("vc_visible-item vc_grid_filter-item "), this.filterValue = filter, this.$content.find(".vc_grid-item" + this.filterValue).addClass("vc_grid_filter-item"), this.showItems(animate)
    }, vcGridStyleLoadMore.prototype.addItems = function(html) {
        var self;
        html && html.length && (html = $(html), this.$el.html(html), this.unsetIsLoading(), this.$content = html.find('[data-vc-grid-content="true"]'), this.$loadMoreBtn = html.find('[data-vc-grid-load-more-btn="true"] .vc_btn3'), this.$loadMoreBtn.length || (this.$loadMoreBtn = html.find('[data-vc-grid-load-more-btn="true"] .vc_btn')), (self = this).$loadMoreBtn.on("click", function(e) {
...
function($) {
    ...
    ! function($) {
        ...
        window.VcGridInfinite = function($el, options) {
            ...
            opts.element = $el, opts.handler = function(direction) {
        ...
        var self;
        this.$content.find('.vc_grid_filter-item:not(".vc_visible-item")').length && (self = this, _.defer(function() {
...
function($) {
    ...
    vcGridStylePagination.prototype.init = function() {
        ...
        for (var tempArray, chunk = parseInt(this.settings.items_per_page, 10), i = 0, j = $html.length; i < j; i += chunk) tempArray = $html.slice(i, i + chunk), $('<div class="vc_pageable-slide-wrapper">').append($(tempArray)).appendTo(this.$content);
        ...
        var that, $vcCarousel;
        $.fn.vcOwlCarousel && (($vcCarousel = (that = this).$content.data("owl.vccarousel")) && $vcCarousel.destroy(), this.$content.on("initialized.owl.vccarousel", function(event) {
            var $pagination, $carousel = event.relatedTarget,
            ...
            }), -1 < that.settings.paging_design.indexOf("pagination") && (event = $carousel.items().length, ($pagination = $("<div></div>").addClass("vc_grid-pagination").appendTo(that.$el)).twbsPagination({
                totalPages: event,
                visiblePages: that.settings.visible_pages,
                onPageClick: function(event, page) {
                    $carousel.to(page - 1), window.setTimeout(function() {
                        var activeSlide = $carousel.$element.find(".owl-item.active");
                        activeSlide.length && activeSlide.offset().top < $(window).scrollTop() && $("html, body").animate({
...
function($) {
    ...
    vcGridStyleAllMasonry.prototype = _.extend({}, window.vcGridStyleAll.prototype, {
        ...
        filter: function(filter) {
            if (filter = _.isUndefined(filter) || "*" === filter ? "" : filter, this.filterValue == filter) return !1;
            this.filterValue = filter, this.$content.data("masonry") && this.$content.masonry("destroy"), this.masonryEnabled = !1, this.$content.find(".vc_visible-item").removeClass("vc_visible-item"), this.$content.find(".vc_grid-item" + this.filterValue), this.filtered = !0, $(window).on("resize", this.setMasonry), this.setMasonry(), this.showItems()
        },
        ...
        setMasonry: function(e) {
            var _this, animation, settings;
            window.innerWidth < window.vcGridSettings.mobileWindowWidth ? (this.$content.data("masonry") && this.$content.masonry("destroy"), this.masonryEnabled = !1) : this.masonryEnabled ? (this.$content.masonry("reloadItems"), this.$content.masonry("layout")) : (animation = this.$content.closest(".vc_grid-container").data("initial-loading-animation"), settings = {
                ...
                opacity: 1,
                transform: "scale(1)"
            }), this.$content.masonry(settings), e || (_this = this, setTimeout(function() {
                _this.$content.masonry(settings)
            }, 100)), this.masonryEnabled = !0)
        }
...
function($) {
    ...
    vcGridStyleLazyMasonry.prototype = _.extend({}, window.vcGridStyleLazy.prototype, {
        ...
        filter: function(filter) {
            if (filter = _.isUndefined(filter) || "*" === filter ? "" : filter, this.filterValue == filter) return !1;
            this.$content.data("masonry") && this.$content.masonry("destroy"), this.masonryEnabled = !1, this.$content.find(".vc_visible-item, .vc_grid_filter-item").removeClass("vc_visible-item vc_grid_filter-item"), this.filterValue = filter, this.$content.find(".vc_grid-item" + this.filterValue).addClass("vc_grid_filter-item"), this.filtered = !0, $(window).on("resize", this.setMasonry), this.setMasonry(), _.defer(this.showItems)
        },
        setItems: function(els) {
            this.masonryEnabled ? this.$content.masonry("appended", els) : this.setMasonry()
        },
...
function($) {
    ...
    (VcGrid = function(el) {
        ...
        window.vc_iframe ? (this.$el.off("click.zonePostLink").on("click.zonePostLink", "[data-vc-link]", function() {
            ...
            e = $(this);
            e.hasClass("vc-gitem-link-ajax") || window.open(e.attr("href"))
            ...
            $this.hasClass("vc-gitem-link-ajax") && (e.preventDefault(), httpRequest = new XMLHttpRequest, $this.addClass("vc-spinner"), httpRequest.open("GET", $this.attr("href"), !0), httpRequest.send(), removeSpinner = function() {
                $this.removeClass("vc-spinner vc-spinner-complete vc-spinner-failed")
            }, timeout = 0, httpRequest.onreadystatechange = function() {
        ...
        var filter;
        if (e && e.preventDefault && e.preventDefault(), this.style && $.camelCase("filter-" + this.style), filter = (e = $(e.currentTarget).find("[data-vc-grid-filter-value]")).data("vcGridFilterValue"), e.hasClass("vc_active")) return !1;
        this.$filter.find(".vc_active").removeClass("vc_active"), this.$filterDropdown.find(".vc_active").removeClass("vc_active"), this.$filterDropdown.find('[value="' + filter + '"]').addClass("vc_active").attr("selected", "selected"), e.parent().addClass("vc_active"), this.gridBuilder.filter(filter)
        ...
        _.isUndefined(data) && (data = {}), data = _.extend({
