...
! function($, window, document) {
    ...
    Owl.Defaults = {
        ...
        filter: ["width", "items", "settings"],
        run: function(cache) {
            var css = this.settings.margin || "",
                grid = !this.settings.autoWidth,
                rtl = this.settings.rtl,
                css = {
                    width: "auto",
                    "margin-left": rtl ? css : "",
                    "margin-right": rtl ? "" : css
                };
        ...
        filter: ["width", "items", "settings"],
        run: function() {
            var css = this.settings.stagePadding,
                coordinates = this._coordinates,
                css = {
                    width: Math.ceil(Math.abs(coordinates[coordinates.length - 1])) + 2 * css,
                    "padding-left": css || "",
                    "padding-right": css || ""
                };
            this.$stage.css(css)
        ...
        }), delete(settings = $.extend({}, this.options, overwrites[match])).responsive, settings.responsiveClass && this.$element.attr("class", this.$element.attr("class").replace(new RegExp("(" + this.options.responsiveClass + "-)\\S+\\s", "g"), "$1" + match))) : settings = $.extend({}, this.options), null !== this.settings && this._breakpoint === match || (this.trigger("change", {
            property: {
                name: "settings",
                value: settings
            }
        ...
        var minimum = null,
            maximum = null,
            pull = this.difference(this._drag.pointer, this.pointer(event)),
            stage = this.difference(this._drag.stage.start, pull);
    ...
    }, Owl.prototype.onDragEnd = function(direction) {
        var delta = this.difference(this._drag.pointer, this.pointer(direction)),
        ...
        return this.settings.freeDrag || $.each(coordinates, $.proxy(function(index, value) {
            return value - 30 < coordinate && coordinate < value + 30 ? position = index : this.op(coordinate, "<", value) && this.op(coordinate, ">", coordinates[index + 1] || value - width) && (position = "left" === direction ? index + 1 : index), -1 === position
        ...
        animate && (this.enter("animating"), this.trigger("translate")), this.is("animating") && this.onTransitionEnd(), $.support.transform3d && $.support.transition ? this.$stage.css({
    ...
    }, Owl.prototype.normalize = function(position, m) {
        var n = this._items.length,
            m = m ? 0 : this._clones.length;
        return !$.isNumeric(position) || n < 1 ? position = void 0 : (position < 0 || n + m <= position) && (position = ((position - m / 2) % n + n) % n + m / 2), position
        ...
        if (settings.loop) maximum = this._clones.length / 2 + this._items.length - 1;
            ...
            for (; 1 < maximum - i;) Math.abs(this._coordinates[j = maximum + i >> 1]) < boundary ? i = j : maximum = j;
        ...
        var current = this.current(),
            ...
            revert = this._items.length,
            minimum = this.minimum(),
            maximum = this.maximum();
        this.settings.loop ? (!this.settings.rewind && Math.abs(distance) > revert / 2 && (distance += -1 * direction * revert), (revert = (((position = current + distance) - minimum) % revert + revert) % revert + minimum) !== position && revert - distance <= maximum && 0 < revert - distance && this.reset(current = (position = revert) - distance)) : position = this.settings.rewind ? (position % (maximum += 1) + maximum) % maximum : Math.max(minimum, Math.min(maximum, position)), this.speed(this.duration(current, position, speed)), this.current(position), this.$element.is(":visible") && this.update()
        ...
        var status = {
            ...
            event = $.Event([name, "owl", namespace || "vccarousel"].join(".").toLowerCase(), $.extend({
                relatedTarget: this
            }, status, data));
        ...
        return this.each(function() {
            ...
            data || (data = new Owl(this, "object" == typeof option && option), $this.data("owl.vccarousel", data), $.each(["next", "prev", "to", "destroy", "refresh", "replace", "add", "remove"], function(i, event) {
                data.register({
                    type: Owl.Type.Event,
                    name: event
...
function($, window) {
        ...
        this._core = carousel, this._loaded = [], this._handlers = {
            "initialized.owl.vccarousel change.owl.vccarousel": $.proxy(function(e) {
                if (e.namespace && this._core.settings && this._core.settings.lazyLoad && (e.property && "position" == e.property.name || "initialized" == e.type))
                    for (var settings = this._core.settings, n = settings.center && Math.ceil(settings.items / 2) || settings.items, i = settings.center && -1 * n || 0, position = (e.property && e.property.value || this._core.current()) + i, clones = this._core.clones().length, load = $.proxy(function(i, v) {
                            this.load(v)
    ...
    Lazy.Defaults = {
        lazyLoad: !1
    }, Lazy.prototype.load = function($elements) {
        var $item = this._core.$stage.children().eq($elements),
            $elements = $item && $item.find(".owl-lazy");
        !$elements || -1 < $.inArray($item.get(0), this._loaded) || ($elements.each($.proxy(function(index, image) {
            var $element = $(image),
...
function($, document) {
    ...
    Video.Defaults = {
        ...
        if (-1 < (id = url.match(/(http:|https:|)\/\/(player.|www.)?(vimeo\.com|youtu(be\.com|\.be|be\.googleapis\.com))\/(video\/|embed\/|watch\?v=|v\/)?([A-Za-z0-9._%-]*)(\&\S+)?/))[3].indexOf("youtu")) type = "youtube";
        else {
            if (!(-1 < id[3].indexOf("vimeo"))) throw new Error("Video URL not supported.");
            type = "vimeo"
        }
        ...
        "youtube" === video.type ? (path = "https://img.youtube.com/vi/" + video.id + "/hqdefault.jpg", create(path)) : "vimeo" === video.type && $.ajax({
            type: "GET",
    ...
    }, Video.prototype.play = function(height) {
        var html, item = $(height.target).closest("." + this._core.settings.itemClass),
...
function($) {
    ...
    Navigation.Defaults = {
        ...
        var settings = this._core.settings,
            difference = this._core.items().length <= settings.items,
            index = this._core.relative(this._core.current()),
            loop = settings.loop || settings.rewind;
        this._controls.$relative.toggleClass("disabled", !settings.nav || difference), settings.nav && (this._controls.$previous.toggleClass("disabled", !loop && index <= this._core.minimum(!0)), this._controls.$next.toggleClass("disabled", !loop && index >= this._core.maximum(!0))), this._controls.$absolute.toggleClass("disabled", !settings.dots || difference), settings.dots && (difference = this._pages.length - this._controls.$absolute.children().length, settings.dotsData && 0 != difference ? this._controls.$absolute.html(this._templates.join("")) : 0 < difference ? this._controls.$absolute.append(new Array(1 + difference).join(this._templates[0])) : difference < 0 && this._controls.$absolute.children().slice(difference).remove(), this._controls.$absolute.find(".active").removeClass("active"), this._controls.$absolute.children().eq($.inArray(this.current(), this._pages)).addClass("active"))
    ...
    }, Navigation.prototype.to = function(position, speed, length) {
        length ? $.proxy(this._overrides.to, this._core)(position, speed) : (length = this._pages.length, $.proxy(this._overrides.to, this._core)(this._pages[(position % length + length) % length].start, speed))
...
function($, window) {
    ...
    function Hash(carousel) {
        this._core = carousel, this._hashes = {}, this.$element = this._core.$element, this._handlers = {
            ...
            "changed.owl.vccarousel": $.proxy(function(hash) {
                var current;
                hash.namespace && "position" === hash.property.name && (current = this._core.items(this._core.relative(this._core.current())), (hash = $.map(this._hashes, function(item, hash) {
                    return item === current ? hash : null
                }).join()) && window.location.hash.slice(1) !== hash && (window.location.hash = hash))
            }, this)
            ...
            var position = window.location.hash.substring(1),
                ...
                position = this._hashes[position] && items.index(this._hashes[position]);
...
function($) {
    ...
    }() || ($.support.transition = String(test("transition", !0)), $.support.transition.end = events.transition.end[$.support.transition]), tests_cssanimations() && ($.support.animation = String(test("animation", !0)), $.support.animation.end = events.animation.end[$.support.animation]), tests_csstransforms() && ($.support.transform = String(test("transform", !0)), $.support.transform3d = tests_csstransforms3d())
}(window.Zepto || window.jQuery, (window, document));